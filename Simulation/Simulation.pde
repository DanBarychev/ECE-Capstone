...
float actualX = -0.5; // Input: actual landing location coordinates (m)    
float actualY = 1.42; // Input
float actualT = 0.65; // Input: actual time of flight (s)

float computationTime = 0.3; // Input: AHRS algorithm computation time

float mappedActualT = actualT * fps; // in frames

float vy = 2.5;   // Input: horizontal velocity in forward direction (m s^-1)       vx of AHRS
float vx = -0.8;  // Input: horizontal velocity in left/right direction (m s^-1)    vy of AHRS
float vz = 1.3;   // Input: vertical velocity (m s^-1)                              vz of AHRS

float phiDeg = -5;                  // Input: horizontal angle (deg)
float phi = (PI / 180) * phiDeg;    // horizontal angle (rad)

float hThrow = 1;                   // Input: height ball at release (m)
...
float dispX;
float dispY;
float alpha;             // angle of ball's trajectory due to vx
...
void predictLandingLocation () {
  ...
  if (dispY == 0) dispY = 0.000000001;    
  alpha = atan(dispX / dispY);             // angle of ball's trajectory due to vx
  ...
  mappedX = (viewWidth / 2) + (predictedX / 2) * viewWidth; // predicted x and y landing locations mapped onto screen
  mappedY = height * (1 - (predictedY / 2));  

  mappedActualX = (viewWidth / 2) + (actualX / 2) * viewWidth; 
  mappedActualY = height * (1 - (actualY / 2));     
} 
...
void initRobotData() {
  ...
  robotReachDiameter = viewWidth; 
  robotStartX = viewWidth / 2;
  robotStartY = height / 2;
  robotX = robotStartX;
  robotY = robotStartY;
  robotDiameter = (1/8.0) * viewWidth; 
  isLocReached = false;
  dirChanged = false;
  move = false;
  robotReturnY = 530;
...
void drawLandings() {
   ...
   fill(255, 0, 0);
   textSize(20);
   text("La", mappedActualX-10, mappedActualY+8); // La symbol at actual landing location
}
